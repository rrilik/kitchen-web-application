...
exports.getProduct = asyncHandler(async (req, res, next) => {
  ...
  if (!product) throw new AppError('Продукти з цим ідентифікатором не знайдено', 404);
...
const validateProductImages = (req) => {
  ...
  if (!req.files) {
    throw new AppError('Зображення полів не можна залишати порожніми.', 400);
  } else if (req.files.length < 3) {
    throw new AppError('Має бути мінімум 3 фотографії', 400);
  }
};
...
exports.deleteProduct = asyncHandler(async (req, res, next) => {
  ...
  if (!product) throw new AppError('Немає даних із цим ідентифікатором', 404);
...
exports.updateProduct = asyncHandler(async (req, res, next) => {
  ...
  if (!oldProduct) throw new AppError('Немає даних із цим ідентифікатором', 404);
  ...
  if (deleteImg) {
    [deleteImg].flat().forEach((val) => {
      if (
        ...
        throw new AppError('я тебе не чую', 400);
      }
    });
  ...
  if (req.files) {
    if (req.files.length + oldImgDir.length < 3) {
      return res.status(400).json({
        success: false,
        message: 'Має бути мінімум 3 фотографії',
      });
    }
    await Promise.all(
