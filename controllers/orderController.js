...
exports.getOrder = asyncHandler(async (req, res, next) => {
  ...
  if (!order) throw new AppError('Замовлення з цим ідентифікатором не знайдено', 404);
...
exports.updateOrder = asyncHandler(async (req, res, next) => {
  ...
  if (order.status !== getOrderStatus('PENDING'))
    throw new AppError(
      'Інформація не може бути відредагована, тому що замовлення затверджено!',
      400
    );
...
exports.deleteOrder = asyncHandler(async (req, res, next) => {
  ...
  if (!order) throw new AppError('Замовлення з цим ідентифікатором не існує', 404);
